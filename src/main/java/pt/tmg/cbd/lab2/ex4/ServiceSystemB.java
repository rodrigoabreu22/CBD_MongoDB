...
public class ServiceSystemB {
    ...
    public static void registarProduto(String username, String produto, int quantity, MongoCollection<Document> collection) {
        ...
        if (quantity > limit) {
            System.err.println("The quantity you chose exceeds the maximum number (" + limit + ") of purchasable products in " + timeslot + " seconds.\n");
            return;
        }
        ...
        if (!newUser) {
            ...
            int recentQuantity = compras.stream()
                    .filter(compra -> (currentTime - compra.getLong("time")) <= timeslot)
                    .mapToInt(compra -> compra.getInteger("quantity", 0))
                    .sum();

            if (recentQuantity + quantity > limit) {
        ...
        collection.updateOne(Filters.eq("username", username),
                Updates.push("compras", new Document("produto", produto)
    ...
    public static void productPurchaseLoop(Scanner sc, String username, MongoCollection<Document> collection){
        ...
        availableProdsMenu();
        System.out.println("Insert the id of the product to buy and quantity [{id} {quantity}]. (Type 'exit' to quit)");
        ...
        while (sc.hasNextLine()){
            ...
            if (line.equals("exit")) {
                ...
                int id = getProductId(product[0]);
                int quantity = getProductQuantity(product);

                if (product.length>2 || id == 0 || quantity == 0){
                    System.out.println("Error. You must insert a valid client id and quantity.\n ");
                    System.out.println("Insert the id of the product to buy and quantity [{id} {quantity}]. (Type 'exit' to quit)");
                    continue;
                }
                registarProduto(username, available_prods[id-1], quantity, collection);
            }
    ...
    public static int getProductQuantity(String[] product){
        if (product.length==1){
            ...
            try {
                return Integer.parseInt(product[1]);
            } catch (NumberFormatException e) {
                return 0;
            }
        }
